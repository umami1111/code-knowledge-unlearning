...
def codeparrot(prompts, model_name, output_paths, num_prompts_per_gen=1):
    if type(prompts) == str:
        assert type(output_paths) == str
        prompts = [prompts]
        output_paths = [output_paths]
    assert len(prompts) == len(output_paths)
    pipe = pipeline("text-generation", model=model_name, pad_token_id=50256, device=0, batch_size=num_prompts_per_gen)
    ...
    outputs = pipe(prompts, num_return_sequences=NUM_PER_PROMPT, max_length=256)
        ...
        if filename.endswith(".c"):
            ...
            with open(prompt_path + filename, "r") as f:
                ...
                if len(prompts) == num_prompts_per_gen:
                    codeparrot(prompts, model_name, output_paths, num_prompts_per_gen=num_prompts_per_gen)
                    prompts = []
                    output_paths = []
